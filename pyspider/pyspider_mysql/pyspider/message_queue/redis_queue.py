...
class RedisQueue(object):
    ...
    def push_nowait(self, obj):
        if self.lazy_limit and self.last_qsize < self.maxsize:
            pass
        elif self.full():
            raise self.Full
        self.last_qsize = self.redis.rpush(self.name, obj)
        return True
    
    def push(self, obj, block=True, timeout=None):
        if not block:
            return self.push_nowait()

        start_time = time.time()
        while True:
            try:
                return self.push_nowait(obj)
            except self.Full:
                if timeout:
                    lasted = time.time() - start_time
                    if timeout > lasted:
